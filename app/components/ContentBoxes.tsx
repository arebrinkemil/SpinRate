  ...
  return (
    <Link to={`/album/${album.id}`} key={album.id} className={className}>
      <CornerMarkings
        ...
        hoverEffect={true}
      >
        <li className="bg-lightsilver flex h-full w-full flex-col p-2 lg:p-4 pr-0">
          ...
          <div className="hidden lg:block">
            {album.name.length > 15 ? (
              <h1 className="text-black text-xl whitespace-nowrap overflow-hidden relative group mr-4">
                <span className="block group-hover:animate-marquee">
                  {album.name}
                </span>
              </h1>
            ) : (
              <h1 className="text-black text-xl mr-4">{album.name}</h1>
            )}

            {album.artist.name.length > 15 ? (
              <p className="text-base md:text-lg  text-black whitespace-nowrap overflow-hidden relative group mr-4">
                <span className="block group-hover:animate-marquee">
                  {album.artist.name}
                </span>
              </p>
            ) : (
              <p className="text-base md:text-lg text-black mr-4">
                {album.artist.name}
              </p>
            )}
          </div>
          <div className="block lg:hidden">
            <h1 className="text-base md:text-lg leading-none">
              {truncateText(album.name, 24)}
            </h1>
            <p className="text-base md:text-lg leading-none text-black">
    ...
    <Link to={`/song/${song.id}`} key={song.id}>
      ...
      >
        <li className="bg-lightsilver flex h-full w-full flex-col p-2 lg:p-4 pr-0">
          ...
          <div className="hidden lg:block">
            {song.name.length > 15 ? (
              <h1 className="text-black text-base md:text-lg whitespace-nowrap overflow-hidden relative group mr-4">
                <span className="block group-hover:animate-marquee">
                  {song.name}
                </span>
              </h1>
            ) : (
              <h1 className="text-base md:text-lg mr-4">{song.name}</h1>
            )}
        ...
        <li className="bg-lightsilver flex h-full w-full flex-col p-2 lg:p-4 pr-0">
          ...
          <div className="hidden lg:hidden">
            {artist.name.length > 15 ? (
              ...
              <h1 className="text-black text-xl mr-4">{artist.name}</h1>
            )}
          </div>
          <div className="hidden lg:block">
            {artist.name.length > 15 ? (
          ...
          <div className="block lg:hidden">
            <h1 className="text-black text-xl">{artist.name}</h1>
          </div>
        </li>
      </CornerMarkings>
    </Link>
