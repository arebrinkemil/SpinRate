...
export function HighlightBox({ item }: { item: any }) {
  return (
    <div className="bg-black col-span-2 row-span-1 lg:col-span-3 lg:row-span-1  2xl:col-span-4 2xl:row-span-2">
      <div className="flex flex-col bg-gray-800 p-4 rounded-lg">
        ...
        <div className="flex flex-row space-x-4">
          {item.highlightIDs.map((highlight: any, index: number) => (
            <Link
              key={`${highlight.text}-${highlight.type}-${index}`}
              to={highlight.url ?? "#"}
            >
              <p className="text-white underline">{highlight.name}</p>
              <p className="text-white underline">{highlight.type}</p>
            </Link>
          ))}
        </div>
      </div>
